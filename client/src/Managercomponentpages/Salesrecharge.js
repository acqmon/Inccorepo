...
const Salepromotionrecharge = () => {
  const navigate = useNavigate();

  const [plans, setPlans] = useState([]);

  const loadplans = () => {
    axios.get("http://localhost:5000/rendersalespromotionplans").then((response) => {
  ...
  const selectplan = (id) => {
    axios.get(`http://localhost:5000/selectsalespromotionplanmanager/${id}`).then(() => {
      navigate(`/selectsalespromotionplans/${id}`);
    });
  };
...
export default Salepromotionrecharge;