...
const Marketersalesrechargesystem = () => {
    ...
    const planslist = () => {
        axios.get("http://localhost:5000/marketersalesrechargelist").then((response) => {
            setPlans(response.data);
        });
    };

    const addplan = () => {
        axios.post("http://localhost:5000/marketersalesrechargesystem", {
    ...
    const selectplan = (id) => {
        axios.get(`http://localhost:5000/selectupdatemarketersalesplan/${id}`).then(() => {
            navigate(`/updatemarketersalesrecharge/${id}`);
        });
    };

    const deleteplan = (id) => {
        axios.delete(`http://localhost:5000/deletemarketersalesplan/${id}`).then(() => {
            planslist();
        });
    };
    ...
    return (
        <div className='table'>
            ...
            <h1 className='headingtable'>Marketer Sales Recharge System</h1>
...
export default Marketersalesrechargesystem;