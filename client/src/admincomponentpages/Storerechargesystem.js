...
const Storerechargesystem = () => {
    ...
    const planslist = () => {
        axios.get("http://localhost:5000/storerechargelist").then((response) => {
            setPlans(response.data);
        });
    };

    const addplan = () => {
        axios.post("http://localhost:5000/storerechargesystem", {
    ...
    const selectplan = (id) => {
        axios.get(`http://localhost:5000/selectupdatestoreplan/${id}`).then(() => {
            navigate(`/updatestorerecharge/${id}`);
        });
    };

    const deleteplan = (id) => {
        axios.delete(`http://localhost:5000/deletestoreplan/${id}`).then(() => {
            planslist();
        });
    };
    ...
    return (
        <div className='table'>
            ...
            <h1 className='headingtable'>Store Recharge System</h1>
...
export default Storerechargesystem;