...
const Rechargehistory = () => {
    ...
    const history = () => {
        axios.post("http://localhost:5000/rechargehistory",{
            storeid: storeid
        }).then((response) => {
                ...
                <tbody>
                    {historyplans.map((historyplan, index) => (
                        <tr key={historyplan.id}>
                            ...
                            <td>{historyplan.rechtype}</td>
                            <td>{historyplan.rechamount}</td>
                            <td>{historyplan.purchbalance}</td>
                            <td>{historyplan.sellingbalance}</td>
                            <td>{historyplan.disc}</td>
                            <td>{historyplan.useridentity}</td>
